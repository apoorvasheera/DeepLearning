...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    - gamma, beta, bn_param: Weights and params for the
...
def affine_norm_relu_backward(dout, cache):
    ...
    da = relu_backward(dout, relu_cache)
    dc, dgamma, dbeta = batchnorm_backward(da, norm_cache)
...
def affine_lnorm_relu_forward(x, w, b, gamma, beta, ln_param):
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    b, lnorm_cache = layernorm_forward(a, gamma, beta, ln_param)
    out, relu_cache = relu_forward(b)
    cache = (fc_cache, lnorm_cache, relu_cache)
    return out, cache

def affine_lnorm_relu_backward(dout, cache):
    """Backward pass for the affine-norm-relu convenience layer.
    """
    fc_cache, lnorm_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)
    dc, dgamma, dbeta = layernorm_backward(da, lnorm_cache)
    dx, dw, db = affine_backward(dc, fc_cache)
    return dx, dw, db, dgamma, dbeta
